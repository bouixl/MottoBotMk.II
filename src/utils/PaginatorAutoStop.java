 ...
 * </ul>
 *
 * Additionally, if specified in the {@link PaginatorAutoStop.Builder}, two "bulk skip" reactions
 * will be added to allow a certain number of pages to be skipped left or right.
 * <br>
 * Paginator.Builders can also set a Paginator to accept various forms of text-input,
 * such as left and right text commands, and even user specified page number via text.
 *
 * @author John Grosh
 */
public class PaginatorAutoStop extends Menu {
	...
	PaginatorAutoStop(EventWaiter waiter, Set<User> users, Set<Role> roles, long timeout, TimeUnit unit, BiFunction<Integer, Integer, Color> color, BiFunction<Integer, Integer, String> text,
		...
		this.strings = items;
		this.pages = (int) Math.ceil((double) this.strings.size() / itemsPerPage);
	 ...
	 * PaginatorAutoStop#paginate(MessageChannel, int)
	 ...
	 * {@link PaginatorAutoStop#paginate(Message, int) Paginator#paginate(Message, int)}.
	 *
	 * @param message
		...
		this.waiter.waitForEvent(GenericMessageEvent.class, event -> {
			if (event instanceof MessageReactionAddEvent)
				...
				if (this.leftText != null && this.rightText != null) {
					if (rawContent.equalsIgnoreCase(this.leftText) || rawContent.equalsIgnoreCase(this.rightText))
			...
			if (event instanceof MessageReactionAddEvent) {
				...
				mre.getMessage().delete().queue(v -> {}, t -> {}); // delete the calling message so
	...
	private void paginationWithoutTextInput(Message message, int pageNum) {
		this.waiter.waitForEvent(MessageReactionAddEvent.class, event -> this.checkReaction(event, message.getIdLong()), // Check
																															// Reaction
				event -> this.handleMessageReactionAddAction(event, message, pageNum), // Handle
																						// Reaction
				this.timeout, this.unit, () -> this.finalAction.accept(message));
	}
	...
	private void handleMessageReactionAddAction(MessageReactionAddEvent event, Message message, int pageNum) {
		...
		switch (event.getReaction().getReactionEmote().getName()) {
			...
			case BIG_LEFT:
				if (newPageNum > 1 || this.wrapPageEnds) {
					for (int i = 1; (newPageNum > 1 || this.wrapPageEnds) && i < this.bulkSkipNumber; i++) {
						if (newPageNum == 1 && this.wrapPageEnds)
							newPageNum = this.pages + 1;
						newPageNum--;
					}
			...
			case BIG_RIGHT:
				if (newPageNum < this.pages || this.wrapPageEnds) {
					for (int i = 1; (newPageNum < this.pages || this.wrapPageEnds) && i < this.bulkSkipNumber; i++) {
						if (newPageNum == this.pages && this.wrapPageEnds)
							newPageNum = 0;
						newPageNum++;
					}
	...
	private Message renderPage(int pageNum) {
		...
		int end = this.strings.size() < pageNum * this.itemsPerPage ? this.strings.size() : pageNum * this.itemsPerPage;
		if (this.columns == 1) {
			...
			int per = (int) Math.ceil((double) (end - start) / this.columns);
	 ...
	 * @author John Grosh
	 */
	public static class Builder extends Menu.Builder<Builder, PaginatorAutoStop> {
		 ...
		 *             <ul>
		 *             <li>No {@link com.jagrosh.jdautilities.commons.waiter.EventWaiter
		 *             EventWaiter} was set.</li>
		 *             <li>No items were set to paginate.</li>
		 *             </ul>
		 */
		@Override
		public PaginatorAutoStop build() {
		 ...
		 *            A BiFunction that uses both current and total page numbers to get a Color for
		 *            the MessageEmbed
		 *
		 * @return This builder
		 */
		...
		/**
		 * Sets the text of the {@link net.dv8tion.jda.api.entities.Message Message} to be
		 * displayed
		 ...
		 *
		 * @param text
		 *            The Message content to be displayed above the embed when the Paginator is
		 *            built
		 *
		 * @return This builder
		 */
		 ...
		 * <br>
		 * As the page changes, the BiFunction will re-process the current page number and the total
		 * page number, allowing for the displayed text of the Message to change depending on the
		 * page number.
		 *
		 * @param textBiFunction
		 *            The BiFunction that uses both current and total page numbers to get text for
		 *            the Message
		 *
		 * @return This builder
		 */
		 ...
		 *
		 * @param show
		 *            {@code true} if the page number should be shown, {@code false} if it should
		 *            not
		 *
		 * @return This builder
		 */
		 ...
		 * timeout, and possibly run a provided {@link java.lang.Runnable Runnable}, if only a
		 ...
		 * provided via {@link PaginatorAutoStop.Builder#setLeftRightText(String, String)} will
		 ...
		 * the {@link net.dv8tion.jda.api.entities.Channel Channel} the menu is displayed in.
		 *
		 * <p>
