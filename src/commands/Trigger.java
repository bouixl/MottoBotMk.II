...
public abstract class Trigger {
	...
	public Trigger(String name) {
		...
		this.name = name;
		this.nsfw = false;
		this.authorizedGuildIds = new ArrayList<String>();
		this.authorizedUserIds = new ArrayList<String>();
		this.blacklistedGuildIds = new ArrayList<String>();
		this.blacklistedUserIds = new ArrayList<String>();
		this.permissionsRequired = new ArrayList<Permission>();
	}

	public Trigger(String name, String shortHelp, String longHelp, List<String> authorizedGuildIds, List<String> authorizedUserIds,
			List<String> blacklistedGuildIds, List<String> blacklistedUserIds, List<Permission> permissionsRequired) {
		super();
		this.name = name;
    ...
    public void run(MottoBot bot, MessageReceivedEvent event) {
    	if(event.getChannelType()==ChannelType.TEXT) // Guild Message
    	{
			...
			if(this.permissionsRequired!=null && !this.permissionsRequired.isEmpty() && (event.getMember().getPermissions().contains(Permission.ADMINISTRATOR) || !event.getMember().getPermissions().containsAll(this.permissionsRequired))) {
